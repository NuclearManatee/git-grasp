...
export function doctor() {
  ...
  if (!dbCheck.ok) {
    ...
    lines.push('  Fix: bun run build-catalog && bun run seed');
  } else {
    lines.push(`DB: OK (${dbCheck.hash.slice(0, 12)}…) schema v${SCHEMA_VERSION}`);
  ...
  const hasModel = existsSync(modelDir)
    ...
    || process.env.GIT_HELP_MOCK_EMBEDDINGS === '1';
  if (!hasModel) {
    ok = false;
    lines.push('Model: MISSING — first real search downloads Xenova/all-MiniLM-L6-v2, or set GIT_HELP_MOCK_EMBEDDINGS=1');
  } else if (process.env.GIT_HELP_MOCK_EMBEDDINGS === '1') {
