...
/** Fenced ``` line or sole inline-code line (tldr-style `git â€¦`). */
...
/**
 * Multi-anchor assignment: floor, Î” from best, hard cap N.
 * Chunks that literally mention a taxonomy command get score at least `floor`
 * so example lines that contain `git â€¦` still route after prose+code binding.
...
function loadSourceTexts() {
  ...
  const docsDirPath = path.join(catalogDir(), 'docs');
  files.push(...walkFiles(docsDirPath, ['.md', '.html', '.txt']));
...
export async function prepareSemanticBlocks(opts = {}) {
  ...
  if (unrouted.length) {
    const sample = unroutedRows
      ...
      .join(', ');
    log(`prepare: unrouted sample origins: ${sample}${unrouted.length > 5 ? 'â€¦' : ''}`);
  }
  log(`prepare: done â†’ ${outPath}`);
