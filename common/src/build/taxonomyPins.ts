// @ts-nocheck
/**
 * LLM-driven taxonomy role tagging + canonical pins (passes Aâ€“D).
...
function defaultMissSummaryPath() {
  return path.join(evalDataDir(), 'last-miss-summary.json');
}
...
export async function passATagRoles(
  ...
  for (const [section, cmds] of bySection) {
    for (let i = 0; i < cmds.length; i += ROLE_TAG_BATCH_SIZE) {
      ...
      log(`Pass A: tagging roles ${section} [${i + 1}â€“${Math.min(i + batch.length, cmds.length)}/${cmds.length}]`);
...
export async function passBDraftPins(
  ...
  for (let i = 0; i < worthy.length; i += BATCH) {
    ...
    const label = `Pass B: draft pins [${i + 1}â€“${Math.min(i + batch.length, worthy.length)}/${worthy.length}] (${batch.map((c) => c.command).join(', ')})`;
    log(label);
    const t0 = Date.now();
    ...
    log(`  ok in ${Date.now() - t0}ms â†’ ${data.pins.length} raw pin(s)`);
...
export async function passCGapFill(
  ...
  const taxonomy_summary = taxonomy.commands
    .map((c) => {
      const roles = (roleMap.get(c.command) || []).join(',');
      return `${c.command} [${roles}] â€” ${c.summary || ''}`;
    })
    .join('\n');
  ...
  log(`Pass C ok in ${Date.now() - t0}ms â†’ ${data.pins.length} new raw pin(s)`);
...
export async function passDRepair(
  ...
  log(`Pass D ok in ${Date.now() - t0}ms â†’ ${data.pins.length} repaired raw`);
  ...
  for (const f of invalid) {
    ...
    // Not returned and not explicitly dropped â†’ still invalid / dropped
    unrepaired.push(f);
  }
...
/**
 * Full Aâ€“D pipeline. Writes roles + canonical pins (valid rows only).
 */
export async function runTaxonomyPins(deps: TaxonomyPinsDeps = {}): Promise<{
  ...
  log(`Wrote ${pins.length} canonical pins (${dropped.length} dropped) â†’ ${pinsOut}`);
