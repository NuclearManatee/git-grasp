 ...
 * Order: GIT_GRASP_ROOT ÔåÆ directory of process.execPath (compiled releases) ÔåÆ
 * cwd ÔåÆ walk from this module ÔåÆ hard fallback to repo root.
 */
...
function findPackageRoot() {
  ...
  // Fallback: packages/core ÔåÆ repo root (../../..)
...
export function buildCacheDir() {
  return path.join(packageDataDir(), 'cache', 'build');
}

/** Durable Step −1 output (do not wipe with build cache). */
export function prepareCacheDir() {
  return path.join(packageDataDir(), 'cache', 'prepare');
}
...
export function taxonomyDir() {
  return path.join(PACKAGE_ROOT, 'packages', 'core', 'taxonomy');
}
