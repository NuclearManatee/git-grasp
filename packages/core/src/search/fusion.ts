...
/** Min-max normalize a batch to [0,1]. Equal values → all 0. */
...
/**
 * C = min(1, S1 * (1 + max(0, S1 - S2))); missing S2 ⇒ 0.
 */
 ...
 *
 * - C > veryHigh (0.9) → 1 result, no alert (exact)
 * - C > high (0.75)    → 2 results, yellow
 * - C > medium (0.4)   → 3 results, orange
 * - else               → 0 results, red
 */
