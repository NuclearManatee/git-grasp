...
export function evalDataDir() {
  ...
  return path.join(PACKAGE_ROOT, 'data', 'eval');
}
...
export function buildCoveragePromoteReport(rows, taxonomyVerbs, opts = {}) {
  ...
  const summary = `coverage ${withMin}/${verbs.length} verbs have ≥${minRecipes} recipes (fraction=${fractionWithMinRecipes.toFixed(2)}${warn ? ' WARN' : ''})`;
  return {
...
/** Write coverage report JSON under data/eval/. */
