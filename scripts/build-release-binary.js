 ...
 *
 * Usage:
 *   bun scripts/build-release-binary.js
 *   bun scripts/build-release-binary.js --out-dir=./dist-release
 *
 * Asset name (no version — stable for releases/latest/download):
 *   git-help-linux-x64.zip | git-help-darwin-arm64.zip | git-help-windows-x64.zip
 */
...
const binName = process.platform === 'win32' ? 'git-help.exe' : 'git-help';
const zipName = `git-help-${slug}.zip`;
...
const entry = path.join(PACKAGE_ROOT, 'apps', 'cli', 'bin', 'index.js');
...
writeFileSync(path.join(outDir, `git-help-${slug}.json`), `${JSON.stringify(meta, null, 2)}\n`);
