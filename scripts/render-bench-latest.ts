#!/usr/bin/env bun
/**
 * Render docs/benchmarks/latest.md from bench/results-{gate,mid,tiny,host}.json when present.
 */
...
function load(key) {
  const p = path.join(root, 'bench', `results-${key}.json`);
...
if (!any) {
  console.error('No bench/results-{gate,mid,tiny,host}.json found; refusing to overwrite latest.md');
  process.exit(1);
}
