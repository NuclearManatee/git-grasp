...
async function warmModel() {
  ...
  const { getEmbedder } = await import('@git-help/core');
...
async function main() {
  ...
  if (!hasTc()) {
    const msg = 'tc/ip not available — install iproute2 or run inside the install bench container';
    if (opts.requireTc) {
      console.error(msg);
      process.exit(1);
    }
  ...
  let ifname = null;
  const work = mkdtempSync(path.join(tmpdir(), 'git-help-install-'));
  try {
    ...
    const install = run('bun', ['install', '--frozen-lockfile'], {
      ...
      env: {
        ...process.env,
        GIT_HELP_SKIP_POSTINSTALL: opts.skipModel ? '1' : '0',
        GIT_HELP_MOCK_EMBEDDINGS: opts.skipModel ? '1' : undefined,
      },
      stdio: 'inherit',
    });
    ...
    mkdirSync(path.join(PACKAGE_ROOT, 'bench'), { recursive: true });
    writeFileSync(
      path.join(PACKAGE_ROOT, 'bench', 'install-last.json'),
