 ...
 *
 * Measures full Bun CLI wall time (cold + warm) against test/performance/queries.json.
 *
 * Usage:
 *   bun run bench
 *   bun run bench -- --mode warm --skill both --iters 50 --discard 5
 *   bun run bench -- --json --out local/bench/results.json
...
import {
  ...
  readConfig,
  DEFAULT_RECALL_K,
} from '@git-grasp/common';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = PACKAGE_ROOT;
const CLI = path.join(ROOT, 'apps', 'cli', 'bin', 'index.ts');
const COMPILED_BASE = path.join(ROOT, 'local', 'bench', 'git-grasp');
const QUERIES_PATH = path.join(ROOT, 'test', 'performance', 'queries.json');
const BUDGET_MS = 500;
...
function ensureCompiled() {
  ...
  console.error(`Compiling CLI ÔåÆ ${outfile}`);
...
function parseArgs(argv) {
  const out = {
    ...
    quick: false,
    queryLimit: null,
    mock: process.env.GIT_GRASP_MOCK_EMBEDDINGS === '1',
    // bun --compile + sharp/transformers is reliable on Linux (Docker gate).
    // Windows defaults to bun-script unless --compile is forced.
    compile: process.env.GIT_GRASP_BENCH_COMPILE === '1'
      || (process.platform !== 'win32' && process.env.GIT_GRASP_BENCH_COMPILE !== '0'),
    noCompile: false,
    sticky: false,
  };
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (a === '--mode') out.mode = argv[++i];
      ...
      console.log(`Usage: bun scripts/bench-search.ts [options]
  ...
  --mock                  GIT_GRASP_MOCK_EMBEDDINGS=1
...
function runCli(query, env = {}, invoker) {
  return new Promise((resolve) => {
    ...
    const child = spawn(invoker.cmd, [...invoker.args, query], {
      cwd: ROOT,
      env: { ...process.env, ...env, GIT_GRASP_MOCK_EMBEDDINGS: env.GIT_GRASP_MOCK_EMBEDDINGS ?? process.env.GIT_GRASP_MOCK_EMBEDDINGS },
...
async function benchStickyWarm({ queries, iters, discard, mock, skillLevel }) {
  if (mock) process.env.GIT_GRASP_MOCK_EMBEDDINGS = '1';
...
async function benchCliMode({ mode, queries, iters, discard, mock, invoker }) {
  const env = mock ? { GIT_GRASP_MOCK_EMBEDDINGS: '1' } : {};
  const samples = [];
  const cycle = [];
...
async function runSynthetic() {
  ...
  function timeRank(factor, label) {
    ...
    for (let i = 0; i < 30; i += 1) {
      ...
      synth
        .slice()
        .sort((a, b) => (b._forcedScore ?? b.score ?? 0) - (a._forcedScore ?? a.score ?? 0))
        .slice(0, thresholds.topK ?? 3);
  ...
  return {
    ...
    note: 'In-process sort on duplicated KNN candidates — latency projection only, not quality',
    ...
    rank: {
      ...
      intents3: timeRank(f3, 'intentsÔëê3'),
      intents5: timeRank(f5, 'intentsÔëê5'),
    },
  };
}
...
function formatTable(report) {
  const lines = [];
  lines.push('# git-grasp search latency');
  ...
  if (report.synthetic) {
    ...
    lines.push(`catalog rows=${s.catalog.rows} examples=${s.catalog.examples} intentsÔëê${s.catalog.intentsPerSkillExample.toFixed(2)}`);
